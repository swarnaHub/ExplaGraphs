...
from relation_model import RobertaForRelationPrediction
from utils_relation import (compute_metrics, output_modes, processors, convert_examples_to_features)

logger = logging.getLogger(__name__)

MODEL_CLASSES = {
    'roberta_relation': (RobertaConfig, RobertaForRelationPrediction, RobertaTokenizer)
}
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating", mininterval=10, ncols=100):
            ...
            with torch.no_grad():
                inputs = {'input_ids': batch[0],
                          ...
                          'relation_label': batch[5]}
                outputs = model(**inputs)
                tmp_eval_loss, relation_logits = outputs[:2]

                eval_loss += tmp_eval_loss.mean().item()
            nb_eval_steps += 1
            if relation_preds is None:
                relation_preds = relation_logits.detach().cpu().numpy()
                if not eval_split == "test":
                    out_relation_label_ids = inputs['relation_label'].detach().cpu().numpy()
            else:
                relation_preds = np.append(relation_preds, relation_logits.detach().cpu().numpy(), axis=0)
                if not eval_split == "test":
                    out_relation_label_ids = np.append(out_relation_label_ids,
                                                     inputs['relation_label'].detach().cpu().numpy(), axis=0)

        eval_loss = eval_loss / nb_eval_steps
        relation_preds = np.argmax(relation_preds, axis=1)

        if not eval_split == "test":
            result = compute_metrics(eval_task, relation_preds, out_relation_label_ids)
            result_split = {}
        ...
        with open(output_pred_file, "w") as writer:
            ...
            for relation_pred in relation_preds:
                writer.write("{}\n".format(processor.get_relation_labels()[relation_pred]))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, eval_split="train"):
    ...
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features(examples, processor.get_relation_labels(), args.max_seq_length,
                                                tokenizer, cls_token=tokenizer.cls_token, sep_token=tokenizer.sep_token)

    all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
    all_input_mask = torch.tensor([f.input_mask for f in features], dtype=torch.long)
    all_segment_ids = torch.tensor([f.segment_ids for f in features], dtype=torch.long)
    all_start_indices = torch.tensor([f.start_indices for f in features], dtype=torch.long)
    all_end_indices = torch.tensor([f.end_indices for f in features], dtype=torch.long)
    all_relation_label = torch.tensor([f.relation_label for f in features], dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_start_indices, all_end_indices, all_relation_label)
...
def main():
    ...
    label_list = processor.get_relation_labels()
    num_labels_relation = len(label_list)
